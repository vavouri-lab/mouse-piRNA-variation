...
# Plot data points as bee swarms.
# Black when there is no known TE variant between the two strains in the cluster.
# Red when BL6 has the TE. 
# Blue when CAST has the TE.
beeswarm(
  myListStrain1,
...
# Plot means as lines.
# Grey when there is no known TE variant between the two strains in the cluster.
# Red when BL6 has the TE.
# Red when BL6 has the TE.
# Blue when CAST has the TE.
lines(
  x = c(1 - 0.2, 1 + 0.2),
  y = c(summary(myres[!(row.names(myres) %in% NE_withTEV), "log2FoldChange"])[4],
        summary(myres[!(row.names(myres) %in% NE_withTEV), "log2FoldChange"])[4]),
  col = "darkgrey",
  lwd = 2
)
...
lines(
  x = c(2 - 0.2, 2 + 0.2),
  y = c(summary(myres[row.names(myres) %in% NE_LINE_strain1plus, "log2FoldChange"])[4],
        summary(myres[row.names(myres) %in% NE_LINE_strain1plus, "log2FoldChange"])[4]),
  col = "red",
  lwd = 2
)
...
lines(
  x = c(2 - 0.2, 2 + 0.2),
  y = c(summary(myres[row.names(myres) %in% NE_LINE_strain2plus, "log2FoldChange"])[4],
        summary(myres[row.names(myres) %in% NE_LINE_strain2plus, "log2FoldChange"])[4]),
  col = "blue",
  lwd = 2
)
...
lines(
  x = c(3 - 0.2, 3 + 0.2),
  y = c(summary(myres[row.names(myres) %in% NE_SINE_strain1plus, "log2FoldChange"])[4],
        summary(myres[row.names(myres) %in% NE_SINE_strain1plus, "log2FoldChange"])[4]),
  col = "red",
  lwd = 2
)
...
lines(
  x = c(3 - 0.2, 3 + 0.2),
  y = c(summary(myres[row.names(myres) %in% NE_SINE_strain2plus, "log2FoldChange"])[4],
        summary(myres[row.names(myres) %in% NE_SINE_strain2plus, "log2FoldChange"])[4]),
  col = "blue",
  lwd = 2
)
...
lines(
  x = c(4 - 0.2, 4 + 0.2),
  y = c(summary(myres[row.names(myres) %in% NE_ERV_strain1plus, "log2FoldChange"])[4],
        summary(myres[row.names(myres) %in% NE_ERV_strain1plus, "log2FoldChange"])[4]),
  col = "red",
  lwd = 2
)
...
lines(
  x = c(4 - 0.2, 4 + 0.2),
  y = c(summary(myres[row.names(myres) %in% NE_ERV_strain2plus, "log2FoldChange"])[4],
        summary(myres[row.names(myres) %in% NE_ERV_strain2plus, "log2FoldChange"])[4]),
  col = "blue",
  lwd = 2
)
...
lines(
  x = c(5 - 0.2, 5 + 0.2),
  y = c(summary(myres[row.names(myres) %in% NE_IAP_strain1plus, "log2FoldChange"])[4],
        summary(myres[row.names(myres) %in% NE_IAP_strain1plus, "log2FoldChange"])[4]),
  col = "red",
  lwd = 2
)
...
lines(
  x = c(5 - 0.2, 5 + 0.2),
  y = c(summary(myres[row.names(myres) %in% NE_IAP_strain2plus, "log2FoldChange"])[4],
        summary(myres[row.names(myres) %in% NE_IAP_strain2plus, "log2FoldChange"])[4]),
  col = "blue",
  lwd = 2
)
...
if (length(NE_LINE_strain1plus) > 0 &
  ...
  print(wilcox.test(myres[row.names(myres) %in% NE_LINE_strain1plus, "log2FoldChange"],
                    myres[row.names(myres) %in% NE_LINE_strain2plus, "log2FoldChange"]))
}

if (length(NE_SINE_strain1plus) > 0 &
    length(NE_SINE_strain2plus) > 0) {
  print(wilcox.test(myres[row.names(myres) %in% NE_SINE_strain1plus, "log2FoldChange"],
                    myres[row.names(myres) %in% NE_SINE_strain2plus, "log2FoldChange"]))
}

if (length(NE_ERV_strain1plus) > 0 &
    length(NE_ERV_strain2plus) > 0) {
  print(wilcox.test(myres[row.names(myres) %in% NE_ERV_strain1plus, "log2FoldChange"],
                    myres[row.names(myres) %in% NE_ERV_strain2plus, "log2FoldChange"]))
}

if (length(NE_IAP_strain1plus) > 0 &
    length(NE_IAP_strain2plus) > 0) {
  print(wilcox.test(myres[row.names(myres) %in% NE_IAP_strain1plus, "log2FoldChange"],
                    myres[row.names(myres) %in% NE_IAP_strain2plus, "log2FoldChange"]))
}
